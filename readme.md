...
### 选模型判据：联合指标（2026-09-19 改）

**保存最优轮 / 早停 / 降 LR 现在统一用 `select_dice = min(根系 Dice, 茎 Dice)`**，
不再是单看根系通道。
...
**为什么用 `min` 而不是加权和**：茎练好后（~0.94）恒高于根系（~0.43），此时
`min ≡ 根系 Dice`，判据与过去**完全一致**（回放 1024 / 1536 两个模型，选中的轮次分毫未变）；
只有茎塌到比根系还低时才顶替它——正好卡住要防的那种情况，且不用另拍一个权重系数。
茎在验证集里没标注时 `per_ch_dice[1] == -1`，自动退回单看根系。
